...
本次修复在验证阶段使用了 `git worktree` 同时检出“修复前基线”和“修复后分支”（`perf/json-download-objecturl`），实现双线并行对比（导出是否成功、内存峰值、是否出现 `Extension context invalidated`）。验证完成后已清理相关 worktree 与临时分支，保持主工作目录干净。
